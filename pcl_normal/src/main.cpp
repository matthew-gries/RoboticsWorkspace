...
int main(int argc, char** argv) {
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cardboard(new pcl::PointCloud<pcl::PointXYZRGB>);
    pcl::PointCloud<pcl::Normal>::Ptr normals(new pcl::PointCloud<pcl::Normal>);


    int ref_res = getPlaneAndNormals(cloud, cloud_cardboard, normals);
    if (ref_res != 0) {
        return -1;
    }
    ...
    viewer->setBackgroundColor(0, 0, 0);
    viewer->addPointCloud<pcl::PointXYZRGB>(cloud, "all");
    viewer->addPointCloud<pcl::PointXYZRGB>(cloud_cardboard, "plane");
    viewer->addPointCloudNormals<pcl::PointXYZRGB, pcl::Normal>(cloud_cardboard, normals, 10, 0.05, "normals");
    viewer->addCoordinateSystem(1.0);
    viewer->initCameraParameters();
